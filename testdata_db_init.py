...
from models import Campaign, Influencer, Post, PostData
...
with db.session() as db:
    ...
    usr_dao = UserDao()
    u1 = usr_dao.create_user(
        db=db, user_schema=UserCreateSchema(email="test", password="test")
    )
    u2 = usr_dao.create_user(
        db=db, user_schema=UserCreateSchema(email="tes2", password="test")
    )
    u3 = usr_dao.create_user(
        db=db, user_schema=UserCreateSchema(email="test3", password="test")
    )
    u4 = usr_dao.create_user(
        db=db, user_schema=UserCreateSchema(email="test4", password="test")
    )
    ...
    influencer_schema = InfluencerCreateSchema(
        **{"name": "some influencer name", "note": "something"}
    )
    influencer1 = Influencer(
        name="influencer 1", note=influencer_schema.note, date_added=added, user_id=u1.id
    )
    influencer2 = Influencer(
        name="influencer 2", note=influencer_schema.note, date_added=added, user_id=u1.id
    )
    influencer3 = Influencer(
        name="influencer 3", note=influencer_schema.note, date_added=added, user_id=u2.id
    )
    influencer4 = Influencer(
        name="influencer 4", note=influencer_schema.note, date_added=added, user_id=u3.id
    )
    ...
    post_create_schema = PostCreateSchema(
        **{"url": "www.something.com", "influencer_id": 0, "campaign_id": 0}
    )
    post1 = Post(
        url="www.post1.com",
        generated_redirect="SOME 123",
        date_added=added,
