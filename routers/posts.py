...
def get_posts_by_campaign(campaign_id: int, user: str = Depends(get_current_user)) -> list[Post]:
...
def get_post_by_id(post_id: int, user: str = Depends(get_current_user)) -> Post:
    with SessionLocal() as db:
...
def create_post(post_create_schema: PostCreateSchema, user: str = Depends(get_current_user)) -> Post:
    with SessionLocal() as db:
