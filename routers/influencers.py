...
def get_influencer_by_id(influencer_id: int, user: str = Depends(get_current_user)) -> Influencer:
    with SessionLocal() as db:
...
def get_influencers_by_campaign(campaign_id: int, user: str = Depends(get_current_user)) -> list[Influencer]:
    with SessionLocal() as db:
...
def create_influencer(influencer_schema: InfluencerCreateSchema, user: str = Depends(get_current_user)) -> Influencer:
    with SessionLocal() as db:
        added = datetime.now()
...
def update_influencer(influencer_id: int, influencer_schema: InfluencerCreateSchema, user: str = Depends(get_current_user)) -> Influencer:
    with SessionLocal() as db:
...
def remove_influencer(influencer_id: int, user: str = Depends(get_current_user)) -> GeneralBoolResponseSchema:
    with SessionLocal() as db:
